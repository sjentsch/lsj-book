...
\section{The $\chi^2$ (chi-square) goodness-of-fit test~\label{sec:goftest}}
...
Again, this is nothing new or interesting. It's just notation. If I say that $O~=~(35, 51, 64, 50)$ all I'm doing is describing the table of observed frequencies (i.e., \texttt{observed}), but I'm referring to it using mathematical notation. 
...
\smallskip
\begin{tabular}{ll}
$H_0$: \hspace*{0.5cm} & $P = (.25, .25, .25, .25)$
\end{tabular} 
\smallskip
...
This gives us a value of \texttt{8.44}.
...
Now that you know what the McNemar test is all about, lets actually run one. The \filename{agpp.csv} file contains the raw data that I discussed previously. The \texttt{agpp} data set contains three variables, an \texttt{id} variable that labels each participant in the data set (we'll see why that's useful in a moment), a \verb#response_before# variable that records the person's answer when they were asked the question the first time, and a \verb#response_after# variable that shows the answer that they gave when asked the same question a second time. Notice that each participant appears only once in this data set. Go to the `Analyses' - `Frequencies' - `Contingency Tables' - `Paired Samples' analysis in jamovi, and move \verb#response_before# into the `Rows' box, and \verb#response_after# into the `Columns' box. You will then get a contingency table in the results window, with the statistic for the McNemar test just below it, see Figure \ref{fig:McNemar}
