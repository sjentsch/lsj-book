...
\begin{itemize} \itemsep -2pt
\item Select \texttt{Factor - Exploratory Factor Analysis} from the main jamovi button bar to open the EFA analysis window (Figure \ref{fig:fa3}).
...
\begin{enumerate} \itemsep -2pt
\item Use the \texttt{Rj} editor to run the EFA in \texttt{R} to the same specification as the one in jamovi (i.e. five factors and oblimin rotation) and compute optimally weighted factor scores. Save the new dataset, with the factor scores, to a file. See Figure \ref{fig:fa10}.
...
Now you can go ahead and undertake further analyses, using either the factor-based scores (a mean scale score approach) or using the optimally-weighted factor scores calculated via the \texttt{Rj} editor. Your choice! For example, one thing you might like to do is see whether there are any gender differences in each of our personality scales. We did this for the Agreeableness score that we calculated using the factor-based score approach, and although the plot (Figure \ref{fig:fa12}) showed that males were less agreeable than females, this was not a significant difference (Man-Whitney $U=5760.5, p=.073$).
...
\begin{itemize} \itemsep -2pt
\item Select \texttt{Factor - Confirmatory Factor Analysis} from the main jamovi button bar to open the CFA analysis window (Figure \ref{fig:cfa2}).
\item Select the 5 \texttt{A} variables and transfer them into the ‘Factors’ box and give then the label “Agreeableness”.
\item Create a new Factor in the ‘Factors’ box and label it “Conscientiousness”. Select the 5 \texttt{C} variables and transfer them into the ‘Factors’ box under the “Conscientiousness” label. 
\item Create another new Factor in the ‘Factors’ box and label it “Extraversion”. Select the 5 \texttt{E} variables and transfer them into the ‘Factors’ box under the “Extraversion” label.
\item Create another new Factor in the ‘Factors’ box and label it “Neuroticism”. Select the 5 \texttt{N} variables and transfer them into the ‘Factors’ box under the “Neuroticism” label.
\item Create another new Factor in the ‘Factors’ box and label it “Openness”. Select the 5 \texttt{O} variables and transfer them into the ‘Factors’ box under the “Openness” label.
...
\item Select \texttt{Factor - Confirmatory Factor Analysis} from the main jamovi button bar to open the CFA analysis window (Figure \ref{fig:MTMM5}).
...
\begin{itemize} \itemsep -2pt
\item Select \texttt{Factor - Reliability Analysis} from the main jamovi button bar to open the reliability analysis window (Figure \ref{fig:rel1}).
\item Select the 5 A variables and transfer them into the ‘Items’ box.
\item Under the “Reverse Scaled Items” option, select variable \texttt{A1} in the “Normal Scaled Items” box and move it across to the “Reverse Scaled Items” box. 
